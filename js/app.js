...
const dbg = function(string, item, color) {
    ...
    if (debug) {
        if ((item != null) && item.name) {
            return console.log(`%cβTorrent:${item.infoHash != null ? 'torrent ' : 'torrent ' + item._torrent.name + ':file '}${item.name}${item.infoHash != null ? ' (' + item.infoHash + ')' : ''} %c${string}`, 'color: #33C3F0', `color: ${color}`)
        } else {
            return console.log(`%cβTorrent:client %c${string}`, 'color: #33C3F0', `color: ${color}`)
        }
    }
}
...
const app = angular.module('BTorrent', ['ngRoute', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ngFileUpload', 'ngNotify'], ['$compileProvider', '$locationProvider', '$routeProvider', function($compileProvider, $locationProvider, $routeProvider) {
...
app.filter('progress', function() {
    return function(num) { `${(100 * num).toFixed(1)}%` } })
